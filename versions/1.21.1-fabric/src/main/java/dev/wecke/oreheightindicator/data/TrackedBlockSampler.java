...
public final class TrackedBlockSampler {
    ...
    public void refresh(MinecraftClient client, Collection<String> requestedKeys) {
        ...
        String nextContext = System.identityHashCode(client.world) + "|" + centerX + "|" + centerZ + "|" + keys;
        long now = System.nanoTime();
        if (nextContext.equals(contextKey) && now < nextRefreshNanos) return;
        ...
        for (String key : requestedKeys) {
            ...
            Registries.BLOCK.getOrEmpty(id).ifPresent(block -> {
                ...
                targets.put(id.toString(), new Target(
                    id.toString(), readableName(id), block.getTranslationKey(),
                    item == Items.AIR ? Items.AIR : item, block
                ));
            });
        }
    ...
    private static Map<String, Profile> sample(MinecraftClient client, List<Target> targets, int centerX, int centerZ) {
        int minY = client.world.getBottomY();
        int maxY = client.world.getTopY() - 1;
        ...
        for (int i = 0; i < targets.size(); i++) targetIndexes.put(targets.get(i).block, i);
        ...
        for (int x = centerX - SAMPLE_RADIUS; x <= centerX + SAMPLE_RADIUS; x += SAMPLE_STEP) {
            for (int z = centerZ - SAMPLE_RADIUS; z <= centerZ + SAMPLE_RADIUS; z += SAMPLE_STEP) {
                for (int y = minY; y <= maxY; y++) {
                    cursor.set(x, y, z);
                    Integer targetIndex = targetIndexes.get(client.world.getBlockState(cursor).getBlock());
                    if (targetIndex != null) counts[targetIndex][y - minY]++;
                }
            }
        }
        ...
        for (int i = 0; i < targets.size(); i++) {
            ...
            sampled.put(target.key, new Profile(
                target.key, target.name, target.translationKey, target.item,
                minY, maxY, normalizedProfile(counts[i])
            ));
        }
    ...
    static float[] normalizedProfile(int[] counts) {
        ...
        for (int i = 0; i < counts.length; i++) {
            ...
            for (int offset = -SMOOTHING_RADIUS; offset <= SMOOTHING_RADIUS; offset++) {
                int source = i + offset;
                if (source >= 0 && source < counts.length) sum += Math.max(0, counts[source]);
            }
            smoothed[i] = sum;
            peak = Math.max(peak, smoothed[i]);
        }
        if (peak <= 0.0f) return smoothed;
    ...
    private static String readableName(Identifier id) {
        ...
        for (String word : id.getPath().split("_")) {
            if (!word.isEmpty()) words.add(word.substring(0, 1).toUpperCase(Locale.ROOT) + word.substring(1));
        }
    ...
    public record Profile(String key, String name, String translationKey, Item item, int minY, int maxY, float[] scores) {}
}
