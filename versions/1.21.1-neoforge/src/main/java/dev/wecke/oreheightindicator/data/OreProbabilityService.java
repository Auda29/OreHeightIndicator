...
public final class OreProbabilityService {
    ...
    public boolean updateIfNeeded(int y) {
        ...
        if (providerRevision != lastProviderRevision || samplerRevision != lastSamplerRevision || scoreBuffer.length != provider.oreCount()) {
            rebuildOreList();
    ...
    private void rebuildOreList() {
        ...
        if (blockSampler != null) {
            for (TrackedBlockSampler.Profile profile : blockSampler.profiles()) {
                if (providerKeys.contains(profile.key())) continue;
                OreChance chance = new OreChance(profile.key(), profile.name(), profile.translationKey(), profile.item(), 0.0f);
                sampledChances.add(chance); sortedChances.add(chance);
