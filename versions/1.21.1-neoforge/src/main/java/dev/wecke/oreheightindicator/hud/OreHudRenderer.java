...
public final class OreHudRenderer {
    ...
    public void render(GuiGraphics context) {
        ...
        for (AnimatedOreRow row : renderRows) {
            ...
            if (showIcons) {
                ...
                context.renderItem(row.icon, textX, iconY);
                textX += ICON_SIZE + ICON_TEXT_GAP;
            }
            context.drawString(textRenderer, Component.literal(row.label), textX, textY, TEXT_COLOR, false);
            ...
            if (showPercent) {
                ...
                context.drawString(textRenderer, Component.literal(percent), percentX, percentY, TEXT_COLOR, true);
            }
        }
    ...
    private List<OreProbabilityService.OreChance> selectDisplayedChances() {
        ...
        for (OreProbabilityService.OreChance chance : probabilityService.sortedChances()) {
            ...
            if (!OreDisplayCatalog.isStandardOre(chance.oreKey()) && config.isMaterialTracked(chance.oreKey())) {
                selected.add(chance);
            }
        }
    ...
    private static void drawTextLine(GuiGraphics context, Font textRenderer, int x, int rowTop, int lineHeight, String line) {
        int textY = rowTop + ((lineHeight - textRenderer.lineHeight) / 2);
        context.drawString(textRenderer, Component.literal(line), x + 4, textY, TEXT_COLOR, false);
    }
