...
public final class TrackedBlockSampler {
    ...
    public void refresh(Minecraft client, Collection<String> requestedKeys) {
        if (client == null || client.player == null || client.level == null) { clear(); return; }
    ...
    private void clear() {
        if (!profiles.isEmpty() || !contextKey.isEmpty()) {
            profiles = Map.of(); contextKey = ""; nextRefreshNanos = 0L; revision++;
        }
    }
    ...
    private static List<Target> resolveTargets(Collection<String> requestedKeys) {
        ...
        for (String key : requestedKeys) {
            ...
            BuiltInRegistries.BLOCK.getOptional(id).ifPresent(block -> {
                ...
                targets.put(id.toString(), new Target(id.toString(), readableName(id), block.getDescriptionId(),
    ...
    private static Map<String, Profile> sample(Minecraft client, List<Target> targets, int centerX, int centerZ) {
        ...
        int maxY = client.level.getMaxBuildHeight() - 1;
        ...
        for (int i = 0; i < targets.size(); i++) {
            ...
            sampled.put(target.key, new Profile(target.key, target.name, target.translationKey, target.item,
    ...
    static float[] normalizedProfile(int[] counts) {
        ...
        for (int i = 0; i < counts.length; i++) {
            ...
            smoothed[i] = sum; peak = Math.max(peak, smoothed[i]);
        }
        if (peak <= 0.0f) return smoothed;
        for (int i = 0; i < smoothed.length; i++) smoothed[i] = Math.max(0.0f, Math.min(1.0f, smoothed[i] / peak));
        return smoothed;
    }
